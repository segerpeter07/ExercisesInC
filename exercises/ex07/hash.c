...
{
    Node* addition;
    int pos = hash_hashable(key) % map->n;
    if(map->lists[pos]) {
        addition = prepend(key, value, map->lists[pos]);
    } else {
        addition = make_node(key, value, NULL);
    }
    map->lists[pos] = addition;
}
...
{
    int pos = hash_hashable(key) % map->n;
    return list_lookup(map->lists[pos], key);
}
